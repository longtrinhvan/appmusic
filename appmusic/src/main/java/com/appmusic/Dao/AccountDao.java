...
public class AccountDao extends ConnectMysql {
	...
	public Account getAccountByID(int id) {
		try {
			...
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery(
					"select * from account, role where account.idrole= role.idrole and  account.id =" + id);
			while (rs.next()) {
				Account account = new Account();
				account.id = rs.getInt("id");
				Role role = new Role();
	...
	public String updateAccount(Account account) {
		try {
			Connection conn = getConnection(DB_URL, USER_NAME, PASSWORD);
			String query = " update account set name = ?, idrole =?, fullname = ?, image = ? where id = ?";
			PreparedStatement preparedStmt = conn.prepareStatement(query);
			preparedStmt.setString(1, account.name);
			preparedStmt.setInt(2, account.role.idrole);
			preparedStmt.setString(3, account.fullname);
			preparedStmt.setString(4, account.image);
			preparedStmt.setInt(5, account.id);
			preparedStmt.execute();
			conn.close();
		} catch (Exception ex) {
			ex.printStackTrace();
